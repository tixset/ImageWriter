{******************************************************************************}
{                                                                              }
{  ImageWriter - Native Windows API Unit                                      }
{                                                                              }
...
{                                                                              }
{  Description:                                                                }
{    Low-level Windows API declarations and device enumeration.               }
{    Provides access to NT native device paths and disk operations.           }
{                                                                              }
{******************************************************************************}
...
uses Dialogs, SysUtils, LogUtils;
...
var
   ...
   ErrorNo : DWORD;
begin
   // Don't initialize Result - let NtOpenFile write to it
   Setup;
   ...
   R := NtOpenFile(@Result, dwDesiredAccess or SYNCHRONIZE, @ObjectAttributes, @Status, dwShareMode, FILE_SYNCHRONOUS_IO_NONALERT);
...
begin
   ...
   begin
      ...
      begin
         SetLength(Data, 1024);
         ZeroMemory(PWideChar(Data), Length(Data) * SizeOf(WideChar));
			DirObjInformation := POBJDIR_INFORMATION(PWideChar(Data));
...
begin
   ...
   begin
      ...
      SetLastError(ErrorNo);
      TLogUtils.Error('Error: ' + IntToStr(Windows.GetLastError) + ' ' + SysErrorMessage(Windows.GetLastError));
...
begin
   ...
   begin
      ...
      begin
         ...
         // Partition0 = entire disk, kept for backward compatibility
         // In the future, partition number can be determined from offset
